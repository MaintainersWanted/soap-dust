...
public class HandlerTest extends TestCase {
	...
	public void testServletProtocolIsSUpportedByURL() throws MalformedURLException, ClassNotFoundException {
		new URL("servlet:"); //should not fail
	}
...
class ExpectTestHeaderWithValuesTest1AndTest2 extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp)
		...
		if (! headerNames.hasMoreElements()) resp.setStatus(400);
		else {
			String name = (String) headerNames.nextElement();
			Enumeration headerValues = req.getHeaders(name);
			if (! "Test".equals(name)) resp.setStatus(401);
			else if (! headerValues.hasMoreElements()) resp.setStatus(402);
			else if (! "Test1".equals((String) headerValues.nextElement())) resp.setStatus(403);
			else if (! headerValues.hasMoreElements()) resp.setStatus(404);
			else if (! "Test2".equals((String) headerValues.nextElement())) resp.setStatus(405);
			else if (headerNames.hasMoreElements()) resp.setStatus(406);
		}
	}
}
